...
[![PDF](https://img.shields.io/badge/%F0%9F%93%84_paper-PDF-d33)](docs/paper.pdf)
[![Report](https://img.shields.io/badge/report-markdown-2f6fec)](docs/PAPER.md)
...
- [x] B-M7  2차 judge fairness check (OpenAI gpt-4o-mini로 동일 레코드 재채점) —
  모든 지표 부호 유지, Claude self-preference 증거 없음. `python -m eval.judge_ablation`
- [x] B-M8  인간 작성 질문 서브셋 (n=15, 5 카테고리, 18편 paper) — 합성 편향 일반화 검증.
  사전 임계값 등록 후 적용. `python -m eval.eval_human`

> 📄 **전체 보고서**: [`docs/paper.pdf`](docs/paper.pdf) (컴파일된 PDF) · [`docs/PAPER.md`](docs/PAPER.md) (Markdown)
> — 방법론 + 정량 결과 + 공정성 분석 (영어 본문 + 한국어 초록, 그림 7개).
> LaTeX 소스: `docs/paper.tex` + `docs/references.bib` (Overleaf에서 XeLaTeX로 재컴파일 가능).
>
> 📝 **블로그 글**: [`docs/BLOG.ko.md`](docs/BLOG.ko.md) (한글, velog·brunch·LinkedIn용) · [`docs/BLOG.md`](docs/BLOG.md) (영문, dev.to·Medium용)
...
>
> **B-M7 2차 judge fairness (Claude vs OpenAI gpt-4o-mini, 같은 답변 재채점)**: 4개
> 지표 모두 agent ≫ baseline 부호 유지. answer_relevancy delta는 +0.576 vs +0.544로 거의
> 동일 → judge-robust. context 지표는 OpenAI가 baseline에 더 관대해 격차 축소되나(+0.45/+0.32)
> 여전히 양수. **Claude self-preference 증거 없음** (OpenAI가 agent에 오히려 더 후함).
>
> **B-M8 인간 작성 질문 서브셋 (n=15, 사전 임계값 적용)**: 4 지표 모두 agent 우위 부호 유지.
> human delta (Claude): F −0.01 / AR +0.48 / **CR +0.43** / CP +0.40. (OpenAI): F +0.05 / AR +0.35 /
> **CR +0.57** / CP +0.33. **context_recall이 두 judge 모두 ≥0.40 강화 임계** ✅ — phrasing·judge·
> 합성편향 셋 다 통과. synth Δ가 human Δ보다 일관되게 큼(~0.10~0.30 축소) → **합성-편의 효과
> 실재함 정량 확인**. 그래도 agent 우위는 견고. 18편 paper, 합성 40과 중복 없음.
>
