...
\textbf{Vector route.} Each corpus doc's \texttt{abstract/intro/related\_work} is chunked (1{,}000 chars, 150 overlap), embedded locally with \texttt{BAAI/bge-small-en-v1.5} (fastembed, no API cost), and stored in Chroma (\textbf{1{,}207 chunks}). Retrieval returns the top $k{=}8$ passages, which an LLM answers from verbatim --- the path that recovers the fact-specific details summaries lose.
...
\begin{tabular}{lccc}
\toprule
Metric & Baseline (best) & Agent & $\Delta$ \\
\midrule
Faithfulness      & 0.853 & 0.872 & \textbf{+0.019} \\
Answer relevancy  & 0.299 & 0.791 & \textbf{+0.493} \\
Context recall    & 0.208 & 0.879 & \textbf{+0.671} \\
Context precision & 0.200 & 0.862 & \textbf{+0.662} \\
\bottomrule
\end{tabular}
\end{table}
...
\label{fig:routes}
\end{figure}
The router sends \textbf{34/40} questions to the vector retriever, 5 to local, and 1 to global (Figure~\ref{fig:routes}) --- consistent with the deliberately fact-specific evaluation set, and with the thesis that dense passage retrieval is the capability missing from the graph baseline. The router is not merely ``always vector'': it still selects local/global where appropriate.
...
\begin{tabular}{lcc}
\toprule
Metric & $\Delta$ at 50p & $\Delta$ at 100p \\
\midrule
Faithfulness      & +0.069 & +0.019 \\
Answer relevancy  & +0.565 & +0.493 \\
Context recall    & +0.637 & \textbf{+0.671} \\
Context precision & +0.600 & \textbf{+0.662} \\
\bottomrule
\end{tabular}
\end{table}

The headline (agent $\gg$ baseline) holds at double the corpus (Table~\ref{tab:scale}, Figure~\ref{fig:scale}), and the context-metric gaps \emph{widen}: as the corpus grows, global/local search degrades faster (more entities and summaries dilute the answer) while vector retrieval scales gracefully. Agent absolute scores dip slightly (answer relevancy 0.891 $\to$ 0.791), the expected effect of more distractor passages. Routing is near-identical across scales (vector 33 $\to$ 34). The faithfulness delta shrinks because baseline faithfulness was never the differentiator.
...
Two conclusions follow. First, the low scores are a \emph{genuine retrieval failure}: community summaries and 1-hop subgraphs do not contain the specific numbers the questions ask, so the model correctly declines --- exactly the gap the agent's passage retrieval closes (a fair, substantive improvement). Second, RAGAS scores a noncommittal answer as \textbf{0} regardless of grounding, so the \emph{magnitude} of the answer-relevancy gap (+0.493) is \emph{amplified} by this penalty. \textbf{Recommendation:} lead with context recall (+0.671) and context precision (+0.662) --- they measure retrieval directly and are independent of answer phrasing --- and present answer relevancy as corroborating, with the noncommittal caveat stated explicitly.
...
\section{Limitations}
\begin{itemize}
  \item \textbf{Single judge, same model family.} Answers and the RAGAS judge are both \texttt{claude-haiku}, risking self-preference bias and inflating absolute scores. The bias applies to both systems, but absolute numbers should be read with caution; a second, different judge would strengthen the claim.
  ...
  \item \textbf{Synthetic questions from the same corpus} may favor retrievable phrasings; there is no human-authored test set.
